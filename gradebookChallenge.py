...
def printStudentAverage(studentIndex):
    studentScoreList = gradebook[studentIndex]
    studentTotal  = 0
    for number in studentScoreList:
        studentTotal += number
    studentAverage = studentTotal/(len(studentScoreList))
    print(f"Student {studentIndex + 1}: {studentAverage:.2f}")

def printAssignmentAverage(assignmentIndex):
    total = 0
    for list in gradebook:
        total += list[assignmentIndex]
    assignmentAverage = total/len(gradebook)
    print(f"Assignment {assignmentIndex + 1}: {assignmentAverage:.2f}")

def main():
    print("Assignment Averages: \n")
    for assignment in range(len(gradebook[0])):
        printAssignmentAverage(assignment)
